...
console.log("API Route Environment Variables:", {
  ...
  PUBLIC_SUPABASE_SERVICE_ROLE_KEY: process.env.PUBLIC_SUPABASE_SERVICE_ROLE_KEY
    ? "SET"
    : "NOT SET",
...
console.log(
  ...
  process.env.GOOGLE_GEMINI_API_KEY ? "YES" : "NO"
);
  ...
  try {
    ...
    try {
      ...
      if (postCheckError) {
        ...
        throw new Error(
          `Post not found or database error: ${postCheckError.message}`
        );
      }
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Cleaned response that failed to parse:",
          cleanedResponse
        );
        console.error(
          "Response contains valid JSON?",
          cleanedResponse.includes("{") && cleanedResponse.includes("}")
        );
        throw new Error(
          `Failed to parse Gemini response: ${parseError.message}. Raw response: ${promptsJson}. Cleaned response: ${cleanedResponse}`
        );
      }
    } catch (geminiError) {
        ...
        try {
          ...
          try {
            ...
            if (!vmResponse.ok) {
              ...
              throw new Error(
                `VM API error: ${vmResponse.status} - ${errorText}`
              );
            }
            ...
            console.log(
              "VM response starts with HTML?",
              txtContent.trim().startsWith("<")
            );

            // Check if response is HTML (error page) instead of expected text
            if (
              txtContent.trim().startsWith("<") ||
              txtContent.includes("<!DOCTYPE") ||
              txtContent.includes("<html")
            ) {
              console.error(
                "VM API returned HTML instead of expected text file"
              );
              console.error("HTML content:", txtContent);
              throw new Error(
                "VM API returned HTML error page instead of image URL"
              );
            }
            ...
            console.log(
              `=== Successfully generated image ${i} for ${promptKey}: ${assetUrl} ===`
            );
          } catch (fetchError) {

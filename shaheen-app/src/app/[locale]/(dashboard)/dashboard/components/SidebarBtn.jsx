...
const SidebarBtn = ({activePage, handleActivePage, href, icon, name , onClick}) => {
  return (
    <div
  ...
  >
    <Link
      ...
      onClick={onClick}
    >
      <button className="flex  gap-2 px-4 cursor-pointer py-3 items-center">
        <Image
          src={icon}
          alt="dashboard"
          width={20}
          height={20}
        />{" "}
        <p className="text-[8F8F8F] font-medium capitalize">{name}</p>
      </button>
    </Link>
  </div>
  )
